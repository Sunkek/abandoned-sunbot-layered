...
class Actualzier(commands.Cog):
    ...
    async def auto_update_ranks(self):
        ...
        for guild in self.bot.settings.keys():
            ...
            basic = self.bot.settings[guild].get("rank_basic_member_role_id")
            basic = guild.get_role(basic)
            active = self.bot.settings[guild].get("rank_active_member_role_id")
            active = guild.get_role(active)
            active_activity = self.bot.settings[guild].get("rank_active_member_required_activity")
            active_days = self.bot.settings[guild].get("rank_active_member_required_days", 0)
            active_days = datetime.now() - datetime.timedelta(days=active_days)
            junior = self.bot.settings[guild].get("rank_mod_junior_role_id")
            junior = guild.get_role(junior)
            senior = self.bot.settings[guild].get("rank_mod_senior_role_id")
            senior = guild.get_role(senior)
            admin = self.bot.settings[guild].get("rank_mod_admin_role_id")
            admin = guild.get_role(admin)
            # Active member role
            if (active and active_activity) or (active and active_days):
                eligible_members = await rest_api.get_active_members(self.bot, guild)
